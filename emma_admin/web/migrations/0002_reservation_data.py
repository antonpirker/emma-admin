...
from web.models import BOOKING_STATUS_CHOICES, PROPERTY_CODE_BER, RATE_PLAN_CHOICES, UNIT_GROUP_CHOICES
...
def load_reservation_data(apps, schema_editor):
    ...
    with open(settings.DATA_DIR / 'reservation_report.csv', newline='') as csvfile:
        ...
        for row in reservation_reader:
            # TODO: catch error when duplicate
            print(row)
            ...
            Booking.objects.create(
                customer=Customer.objects.get(first_name=row[FIRST_NAME], last_name=row[LAST_NAME], birth_date=parser.parse(row[BIRTH_DATE])),
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(load_reservation_data, reverse_code=migrations.RunPython.noop),
    ]
