...
from web.models import (BOOKING_STATUS_TO_SLUG, PROPERTY_CODE_TO_NAME, RATE_PLAN_TO_SLUG,
                        UNIT_GROUP_TO_SLUG)
...
def load_reservation_data(apps, schema_editor):
    ...
    Property = apps.get_model('web', 'Property')
    ...
    with open(settings.DATA_DIR / 'reservation_report.csv', newline='') as csvfile:
        ...
        for row in reservation_reader:
            ...
            if not Property.objects.filter(code=row[PROPERTY_CODE]).exists():
                Property.objects.create(
                    code=row[PROPERTY_CODE],
                    name=PROPERTY_CODE_TO_NAME[row[PROPERTY_CODE]],
                )

            if not Unit.objects.filter(unit=row[UNIT]).exists():
                Unit.objects.create(
                    property=Property.objects.get(code=row[PROPERTY_CODE]),
